...
if (is_file($installFile)) {
  ...
  switch ($step) {
    ...
    <?php
    $error = FALSE;
    if(!is_writeable($indexFile)){$error = TRUE; echo "<div class='alert alert-error'><i class='icon-remove'></i> Index File (index.php) is not writeable!</div>"; }
  ...
  <?php if($_POST){
    ...
    if ($object->status == 'success') {
      ...
      if ($database->create_tables($dbdata, $_POST['username'], $_POST['code']) == false) {
        ...
        $finished = TRUE;
        if(!@unlink('../SF')){
          echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the SF file from the main folder in order to lock the installer.</div>";
        }
