...
<div class="row half-gutters align-middle">
  <div class="column mobile-100">
      ...
      Transactions logs records all the credit and debit cases. This is mainly for reference purposes or keep track or verify with the actual transactions numbers in withdrawals, deposits, or manual credits etc..
  </div>
...
Search Form</br>
<div class="searchsr">
      <input type="text" class="esearch" id="esearch" placeholder="Search by user ID or Type or agent id etc..">
...
$("#ajax-content").empty().append("<div id='loading'><img src='<?php echo SITEURL;?>/shell/images/ajax-loader.gif' alt='Loading' /></div>");
      $.ajax({
			type: "POST",
		    url: "<?php echo SITEURL;?>/shell/adm/payments/trans_logs",
			data: {
			usid:vevent,	
			method:"xpcreditrec"
			},
	      ...
	      var dt2 = $('#my_date_picker2').val();
		   ...
		   success: function(response) {
			$("#ajax-content").empty().append(response);
			 var ccom = $('.foncounter').html();
			 $('.countrec').html(ccom);
			 $('#loadid').removeClass('lodo');
			 $('#loadid').addClass('lodoa');
               }
		      });
			   return false;
		});
			 ...
			 //date filter more
     $('body').on('click', ' .lodoa', function(){
	  $(this).html("<div class='oftr'>Loading.......</div>");
	      var dt1 = $('#my_date_picker1').val();
	      var dt2 = $('#my_date_picker2').val();
		  var rowCount = $('.hoper').html();
		   $.ajax({
