...
$("#ajax-content").empty().append("<div id='loading'><img src='<?php echo SITEURL;?>/shell/images/ajax-loader.gif' alt='Loading' /></div>");
      $.ajax({
			type: "POST",
		    url: "<?php echo SITEURL;?>/shell/admin/in_settled_set",
			data: {
  ...
  $('body').on('click', ' .tdrk', function(){
	...
	//search events
	 $("body").on('change', 'input#esearch', function() {
	  ...
	  $.ajax({
			type: "POST",
		    url: "<?php echo SITEURL;?>/shell/admin/in_settled_slips_search",
			data: {
			es:es,
			idt:idt,
			method:"esearch"
    ...
    $('body').on('click', ' .addfilter', function(){
	  $('#ajax-content').html("Loading.......");
	      ...
	      var dt2 = $('#my_date_picker2').val();
		   $.ajax({
			type: "POST",
		    url: "<?php echo SITEURL;?>/shell/admin/in_settled_slips_datesearch",
			data: {
			dt1:dt1,
			dt2:dt2,
