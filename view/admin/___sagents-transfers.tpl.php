...
<div class="row half-gutters align-middle">
  <div class="column mobile-100">
      <h2>Exchange Brokers/Super Agents Transfer</h2>
      Inter account fund transfers are only applicable to super agents or exchange brokers. You can revoke the transfers even after successful transfers. In that case amount will be resettled. 
  </div>
...
<div class="topingsg">
<h2>Transfers Summary [Amount/No. of Rows]</h2><hr>
...
$("#ajax-content").empty().append("<div id='loading'><img src='<?php echo SITEURL;?>/shell/images/ajax-loader.gif' alt='Loading' /></div>");
      $.ajax({
			type: "POST",
		    url: "<?php echo SITEURL;?>/shell/adm/payments/transfers",
			data: {
			usid:vevent,	
			method:"xpcreditrec"
			},
	...
	success: function(html) {
		...
		$.ajax({
			type: "POST",
		    url: "<?php echo SITEURL;?>/shell/adm/payments/tr_txt_data",
			data: {
			usid:vevent,	
			method:"transfers_data"
			},
	...
	//SHOW cancel
      $('body').on('click', ' .revertt', function(){
		$(this).html('Loading...');
		var pid = $(this).attr('id');
		$(this).addClass('xp');
		var conf = confirm("Do you really want to Revert this transaction?");
		if (conf == false) {
			return;
		}
		$.ajax({
			...
			data: {
				pid:pid,
				method:"revert_transfer"
			},
			success: function (response) {
				$('.revertt.xp').html(response);
				//$('.revertt.xp').removeClass('xp');
