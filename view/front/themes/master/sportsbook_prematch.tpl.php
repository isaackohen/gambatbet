...
<script>
    ...
    $('body').on('click', 'ul#cccrlisidebar li', function () {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo SITEURL;?>/shell/prebook/__update_home.php",
            data: {
                method: 'sport_index',
                btype: 'country',
                cc: cc,
    ...
    $('body').on('click', '.gambabet-categories-slider a', function (e) {
        const eventElement = $(e.target);
        const containerElement = eventElement.parentsUntil('.gambabet-categories-slider').parent();
        const dataType = containerElement.data('type');
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo SITEURL;?>/shell/"+dataType+"/__update_home.php",
            data: {
                method: 'sport_index',
                btype: 'sport',
                kspid: spid,
...
<style>
    * {
    ...
    .livenow > span {
        animation: live 20s infinite ease-in-out;
    }
    ...
    a#lnow:before {
        ...
        left: -12px;
        border-radius: 20px;
        box-shadow: 0 0 7px 1px #75ff00, 0.5rem 0.5rem 30px #1f1f1f;
    }
