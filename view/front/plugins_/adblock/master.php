...
<div class="yoyo fitted plugin segment<?php echo ($conf[0]->alt_class) ? ' ' . $conf[0]->alt_class : null;?>">
  ...
  <?php $ad_content = ($row->image) ? ('<a href="' . $href . '" id="b_' . $row->id . '" title="' . $row->image_alt . '"><img src="' . FPLUGINURL . $row->plugin_id . '/'  . $row->image . '" alt="' . $row->image_alt . '" class="yoyo basic image" /></a>') : Validator::cleanOut($row->banner_html);?>
  <?php echo $ad_content;?>
