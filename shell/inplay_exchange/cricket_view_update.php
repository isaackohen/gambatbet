...
//function to get odd value of bet option
	function getOddFromDatabase($odd_id,$odd_name,$cat_name,$sp, $conn){
		if($sp=="3"){
		$qrs="SELECT bet_option_odd FROM af_inplay_bet_options WHERE bet_option_id=$odd_id AND bet_option_name LIKE '%_$odd_name_%' AND cat_name_in_option='$cat_name'";
		}else{
...
//Fetch on new slip data refresh
  function updateuslip($user_id, $conn){
	...
	if(isset($_POST['get_all_events'])){
		...
		echo json_encode(["cash_data"=>$cash_data,'unsubmitted_keys'=>$unsubmitted_keys]);
	}
