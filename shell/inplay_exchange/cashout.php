...
$shsl = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM sh_sf_tickets_records WHERE slip_id = $slipid AND status = 'awaiting' AND bet_info IS NULL AND type = 'back'"));
...
$sql = "SELECT * from sh_sf_tickets_history WHERE user_id=$user_id AND bet_info IS NULL AND bet_option_id = $_option_id AND bet_option_name = '$_option_name' AND status = 'awaiting' AND type = 'back'";
		...
		if(mysqli_affected_rows($conn) > 0){
			...
			if(!empty($said)){
				...
				if(mysqli_affected_rows($conn) > 0){
				 ...
				 mysqli_query($conn,"DELETE FROM sh_sf_tickets_records WHERE user_id=$user_id AND bet_option_id=$_option_id AND bet_option_name='$_option_name' AND type='back'"); 
				}else{
					echo 'Suspended';
					//echo $conn->error;
				}
