<?php error_reporting(0);include('../db.php');
  ...
  if($_POST['method'] == 'userslist'){
	$sql = "SELECT * FROM users WHERE type = 'Sagent' ORDER BY chips DESC LIMIT 50";
	$prec = "SELECT id FROM users WHERE type = 'Sagent'";
  } else if($_POST['method'] == 'usersmore'){
	$sql = "SELECT * FROM users WHERE type = 'Sagent' ORDER BY chips DESC LIMIT $rc, 50";
  ...
  $sql = "SELECT * FROM users WHERE created >= FROM_UNIXTIME($start) AND created <= FROM_UNIXTIME($end) AND type='Sagent' ORDER BY created DESC LIMIT 500";
 $prec = "SELECT id FROM users WHERE created >= FROM_UNIXTIME($start) AND created <= FROM_UNIXTIME($end) AND type='Sagent'";  
	
 } else  if($_POST['method'] == 'esearch'){
	$es = $_POST['es'];
  $idt = $_POST['idt'];
  if (is_numeric($es)){
	$sql = "SELECT * FROM users WHERE id = $es AND type='Sagent'";
	$prec = "SELECT id FROM users WHERE id = $es AND type='Sagent'";
  } else {
	$sql = "SELECT * FROM users WHERE email LIKE '%".$es."%' AND type='Sagent'";
	$prec = "SELECT id FROM users WHERE email LIKE '%".$es."%' AND type='Sagent'";
  }
        ...
        <div class="row horizontal-gutters align-middle">
          ...
          <div class="column shrink">
            ...
            <div class="yoyo dropdown small menu pointing top-right" id="userDrop_<?php echo $row['id'];?>">
			  ...
			  <a class="item" href="/admin/bet-history/?said=<?php echo $row['id'];?>"><i class="icon history"></i>
              <span class="padding-left">Downline Bet History</span></a>
              ...
              <span class="padding-left">Transactions Logs</span></a>
			   ...
			   <a class="item" href="/admin/sagents-transfers/?usid=<?php echo $row['id'];?>"><i class="icon history"></i>
              <span class="padding-left">Transfers</span></a>
