<?php error_reporting(0);include('../db.php');
  ...
  if($_POST['method'] == 'userslist'){
	$sql = "SELECT * FROM users WHERE type = 'member' ORDER BY chips DESC LIMIT 50";
	$prec = "SELECT id FROM users WHERE type = 'member'";
  ...
  $sql = "SELECT * FROM users WHERE created >= FROM_UNIXTIME($start) AND created <= FROM_UNIXTIME($end) AND type='member' ORDER BY created DESC LIMIT 500";
 $prec = "SELECT id FROM users WHERE created >= FROM_UNIXTIME($start) AND created <= FROM_UNIXTIME($end) AND type='member'";  
	
 } else  if($_POST['method'] == 'esearch'){
	$es = $_POST['es'];
  $idt = $_POST['idt'];
  if (is_numeric($es)){
	$sql = "SELECT * FROM users WHERE id = $es AND type='member'";
	$prec = "SELECT id FROM users WHERE id = $es AND type='member'";
  } else {
	$sql = "SELECT * FROM users WHERE email LIKE '%".$es."%' AND type='member'";
	$prec = "SELECT id FROM users WHERE email LIKE '%".$es."%' AND type='member'";
  }
        ...
        <div class="row horizontal-gutters align-middle">
          ...
          <div class="column shrink">
            ...
            <div class="yoyo dropdown small menu pointing top-right" id="userDrop_<?php echo $row['id'];?>">
			  ...
			  <a class="item" href="/admin/users-credit-history/?usid=<?php echo $row['id'];?>"><i class="icon history"></i>
