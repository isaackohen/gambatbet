...
$ear1 = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(CASE WHEN status = 'winning' THEN winnings - stake END) AS wina, SUM(CASE WHEN status = 'losing'  THEN stake END) AS losea FROM sh_sf_slips_history"));
$ear2 = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(CASE WHEN status = 'winning' THEN winnings - stake END) AS wina, SUM(CASE WHEN status = 'losing'  THEN stake END) AS losea FROM sh_sf_slips_history WHERE FROM_UNIXTIME(date) > (NOW() - INTERVAL 7 DAY)"));
$ear3 = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(CASE WHEN status = 'winning' THEN winnings - stake END) AS wina, SUM(CASE WHEN status = 'losing'  THEN stake END) AS losea FROM sh_sf_slips_history WHERE FROM_UNIXTIME(date) > (NOW() - INTERVAL 1 DAY)"));

//players performance
$pp = "SELECT user_id AS uid, COUNT(CASE WHEN status = 'winning' THEN 1 END) AS winslip, COUNT(CASE WHEN status IN('winning','losing') THEN 1 END) AS totslip, SUM(CASE WHEN status IN('winning', 'losing') THEN stake END) AS stakes, SUM(CASE WHEN status = 'winning'  THEN winnings - stake END) AS won FROM sh_sf_slips_history WHERE status IN('winning', 'losing') AND FROM_UNIXTIME(date) > (NOW() - INTERVAL 30 DAY) GROUP BY user_id ORDER BY won DESC LIMIT 100";
...
<hr>
<h2 style="display:inline-block">Settled Tickets (<?php echo $ddk['ikcount'];?>)</h2><a id="revstat">Expand..</a>
<div class="shstats">
