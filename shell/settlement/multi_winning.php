 ...
 $option_name = $_POST['afc'];
   ...
   if($option_name == 'pre'){
	...
	foreach($rows as $row){
		...
		$rkinfo = mysqli_query($conn, "SELECT strip_cus, chips, promo FROM users WHERE id = '$user_id'");
   ...
   ////////////////////INPLAY////////////////////////////
	...
	foreach($rows as $row){
		...
		//for chips
			...
			$cc = mysqli_query($conn, "INSERT INTO `sh_users_credit_records` (`sl_id`, `u_id`, `bet_info`, `odd`, `stake`, `ab`, `amt`, `af`, `status`, `type`, `debit`, `aid`, `dt`) VALUES ('$slip_id', '$user_id', '$bet_info', '$odd', '$stake', '$ab', '0.00', '$ab', 'losing', '$type', '$debit', '$aid', '$dt')");
			if(mysqli_affected_rows($conn) > 0){
				mysqli_query($conn, "DELETE FROM sh_sf_tickets_records WHERE user_id=$user_id AND slip_id = $slip_id");
				echo 'Done';
			} else {
