...
if(isset($_POST['method']) && $_POST['method'] == 'updateStatus'){
 ...
 //GET PARAMETERS
 $option_id = $_POST['winid'];
 $option_name = $_POST['afc'];
 $sql = "SELECT * from sh_sf_slips_history WHERE bet_info IS NULL AND bet_option_id = '$option_id' AND bet_option_name = '$option_name' AND status = 'awaiting'";
	...
	foreach($rows as $row){
		...
		//amount before/after
		 ...
		 $af = $promo + $stake;	
			...
			$ca = mysqli_query($conn, "UPDATE sh_sf_slips_history SET status = 'canceled' WHERE slip_id = '$slip_id' AND status = 'awaiting'");
			...
			if(mysqli_affected_rows($conn) > 0){
				mysqli_query($conn, "DELETE FROM sh_sf_slips WHERE user_id=$user_id AND bet_info IS NULL AND bet_option_id = $option_id AND bet_option_name = '$option_name' AND status = 'awaiting'");
				if($debit == 'chips'){
