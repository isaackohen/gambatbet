	...
	foreach($rows as $row){
		...
		//amount before/after
		 ...
		 $af = $promo + $stake;	
			...
			$cc = mysqli_query($conn, "INSERT INTO `sh_users_credit_records` (`sl_id`, `u_id`, `bet_info`, `stake`, `ab`, `amt`, `af`, `status`, `type`, `debit`, `aid`, `dt`) VALUES ('$slip_id', '$user_id', '$bet_info', '$stake', '$ab', '$stake', '$af', 'canceled', '$type', '$debit', '$aid', '$dt')");
			if(mysqli_affected_rows($conn) > 0){
