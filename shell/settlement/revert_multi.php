...
if(isset($_POST['method']) && $_POST['method'] == 'updateStatus'){
 ...
 //REVERT WINNING
 if($stt == 'winning'){
	$sql = "SELECT * FROM sh_sf_slips_history WHERE status <>'awaiting' AND slip_id = '$slip_id'";
	...
	foreach($rows as $row){
		...
		//for Lay bet
			...
			mysqli_query($conn, "UPDATE sh_sf_slips_history SET status = 'winning' WHERE slip_id = '$slip_id'");
			 die();
			 }
		...
		$ca = mysqli_query($conn, "UPDATE sh_sf_slips_history SET status = 'awaiting' WHERE slip_id = '$slip_id'");
 ...
 //REVERT LOSING
	...
	foreach($rows as $row){
		...
		if($type == 'lay'){
			...
			mysqli_query($conn, "UPDATE sh_sf_slips_history SET status = 'losing' WHERE slip_id = '$slip_id'");
			 die();
			 }
 ...
 //REVERT CANCELED
	...
	foreach($rows as $row){
		...
		if($type == 'lay'){
			...
			mysqli_query($conn, "UPDATE sh_sf_slips_history SET status = 'canceled' WHERE slip_id = '$slip_id'");
			 die();
			 } 
		  } else {
			echo 'Failed'; die();
...
/////////////for back/sbook//////////////////////////
 ...
 /////////////////////////////////FOR PREMATCH REVERT/////////////////////////////////////////////////
	 ...
	 //REVERT WINNING
 if($stt == 'winning'){
	$sql = "SELECT * FROM sh_sf_tickets_records WHERE status <>'awaiting' AND slip_id = $slip_id";
	$result = $conn->query($sql);
	$rows = mysqli_fetch_all($result);
	foreach($rows as $row){
		$slip_id = $row[0];
