<?php include('../db.php');?>
 ...
 $usid = $_POST['usid'];	 
 $sql = "SELECT * FROM sh_sf_tickets_history WHERE user_id = $usid AND status <> 'awaiting' ORDER BY date DESC LIMIT 20";
 }
 //for prematch 
 if(isset($_POST['method']) && $_POST['method'] == 'pre'){
 $usid = $_POST['usid'];	 
 $sql = "SELECT * FROM sh_sf_slips_history WHERE user_id = '$usid' AND status <> 'awaiting' ORDER BY date DESC LIMIT 20";
 } 
 //for games
 ...
 //date filter inplay
 if(isset($_POST['method']) && $_POST['method'] == 'datefilter'){
 $usid = $_POST['usid'];
 $dt1 = $_POST['dt1'];
 $dt2 = $_POST['dt2'];
 $start = strtotime($dt1);
 ...
 $sql = "SELECT * FROM sh_sf_tickets_history WHERE user_id = $usid AND date >=".$start." AND date <= ".$end." AND status <> 'awaiting' ORDER BY date DESC LIMIT 100";
 }
 
  //date filter prematch
 if(isset($_POST['method']) && $_POST['method'] == 'prefilter'){
 $usid = $_POST['usid'];
 $dt1 = $_POST['dt1'];
 $dt2 = $_POST['dt2'];
 $start = strtotime($dt1);
 ...
 $sql = "SELECT * FROM sh_sf_slips_history WHERE user_id = $usid AND date >=".$start." AND date <= ".$end." AND status <> 'awaiting' ORDER BY date DESC LIMIT 100";
 }
 ...
 if(isset($_POST['method']) && $_POST['method'] == 'lomore'){
	 $rc = $_POST['rc'];
	 $usid = $_POST['usid'];
	 $sql = "SELECT * FROM sh_sf_tickets_history WHERE user_id = $usid AND status <> 'awaiting' ORDER BY date DESC LIMIT $rc, 50";
 }
 ...
 if(isset($_POST['method']) && $_POST['method'] == 'prelomore'){
	 $rc = $_POST['rc'];
	 $usid = $_POST['usid'];
	 $sql = "SELECT * FROM sh_sf_slips_history WHERE user_id = '$usid' AND status <> 'awaiting' ORDER BY date DESC LIMIT $rc, 50";
 }
 ...
 if ($result->num_rows > 1) {
	...
	echo '<div id="lrem" class="loadmox inplay">Load More...</div>';
  }	
