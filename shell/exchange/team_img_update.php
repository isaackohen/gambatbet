...
$tk = "94840-yL6oCP0lnJl8xM";
	...
	var_dump(pull_all_event_ids($conn));

	function insertEvents($event_ids,$tk,$conn){
		...
		foreach ($event_ids as $event_id_key => $event_ids_set) {
			...
			echo 'heyo';
			echo $conn->error;
		$url = "https://api.b365api.com/v1/bwin/result?token=" . $tk . "&event_id=" . implode(",",$event_ids_set);
	...
	insertEvents(pull_all_event_ids($conn),$tk,$conn);
