<?php include_once('../db.php');
	...
	//for prematch validation
	function isPrematch($event_id,$option_id,$option_name,$odd,$spid,$token,$event_type,$conn){
		$valF = mysqli_fetch_assoc(mysqli_query($conn,"SELECT bet_option_odd FROM af_pre_bet_options WHERE bet_option_id=$option_id AND bet_option_name='$option_name' AND bet_option_odd='$odd'"));
		if (!empty($valF['bet_option_odd'])){
			return true;
		}else{
			return false;
		}
	...
	function isValidOddCricket($event_id,$option_id,$option_name,$odd,$spid,$token,$event_type,$conn){
	...
	function isValidOdd($event_id,$option_id,$option_name,$odd,$spid,$token,$event_type,$conn){
		if($event_type=='pre'){
			return isPrematch($event_id,$option_id,$option_name,$odd,$spid,$token,$event_type,$conn);
		}
		if($spid==3){
			return isValidOddCricket($event_id,$option_id,$option_name,$odd,$spid,$token,$event_type,$conn);
		} 
			   ...
			   foreach($unserialized_slip_multiple as $key=>$bet){
				...
				if (!empty($estatus)) {
			         echo 'This event is temporarily disabled.';die();	
					   ...
					   $have_bets=true;
						if(!isValidOdd($event_id,$option_id,$option_name,$odd,$spid,$token,$event_type,$conn)){
							$is_valid=false;
