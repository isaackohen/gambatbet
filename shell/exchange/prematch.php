...
//events insert soccer page

function insertPrematch($pg, $tk, $conn){
$url = 'https://api.b365api.com/v1/bwin/prematch?token='.$tk.'&page='.$pg;
$ch = curl_init($url);
curl_setopt($ch, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4 );
curl_setopt($ch, CURLOPT_TIMEOUT, 100);
curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 100);
...
foreach($obj['results'] as $result){
    $output = $result['Id'];
    $url='https://api.b365api.com/v1/bwin/event?token='.$tk.'&skip_markets=1&event_id='.$output;
    $ch = curl_init($url);
	curl_setopt($ch, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4 );
    curl_setopt($ch, CURLOPT_TIMEOUT, 30);
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
...
$url = 'https://api.b365api.com/v1/bwin/prematch?token='.$tk.'&page=3';
$ch = curl_init($url);
curl_setopt($ch, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4 );
curl_setopt($ch, CURLOPT_TIMEOUT, 100);
curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 100);
