...
if(isset($_POST['method']) && $_POST['method'] == 'sashowvaltotr') {
	...
	$agmin = mysqli_fetch_assoc(mysqli_query($conn,"SELECT sa_min_transfer FROM risk_management"));
	$min_allowed = $agmin['sa_min_transfer'];
	...
	//get agent details
	$getusr= mysqli_fetch_assoc(mysqli_query($conn,"SELECT id,stripe_cus,sabal FROM users WHERE id=$usid"));
	$agent_curr = $getusr['stripe_cus']; //broker rate
	$agent_balance = $getusr['sabal'];
	...
	echo "<div id='wraptramoutn'>You will receive : <span style='color:#009688;font-weight:bold'> ".round($net_transfer_amount,2)." ".$agent_curr. "</span> in your betting account <div class='sasubtransferag'>Transfer Balance</div></div>";
...
//submit transfer
else if(isset($_POST['method']) && $_POST['method'] == 'saagpaynum') {
	$trmoney=$_POST['trmoney'];
	//get agent currency value rate
	function agent_exchange($agent_curr,$conn){
		$query="SELECT rate FROM currencies WHERE name='$agent_curr'";
	...
	//if less than minimum, return
	if($trmoney < $min_allowed){
		echo 'Minimum transfer allowed '.$min_allowed.' BDT';
		die();
	}
	
	
	//get agent details
	$getusr= mysqli_fetch_assoc(mysqli_query($conn,"SELECT id,stripe_cus,chips,sabal FROM users WHERE id=$usid"));
	$agent_curr = $getusr['stripe_cus']; //broker rate
	$agent_balance = $getusr['sabal'];
	...
	$prevrec = $getusr= mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM sh_sf_deposits WHERE user_id=$usid AND type='sa_commission' AND FROM_UNIXTIME(date) > (NOW() - INTERVAL 30 DAY)"));
	if(!empty($prevrec)){
	...
	mysqli_query($conn, "UPDATE users SET sabal = sabal - $trmoney WHERE id=$usid");
	if(mysqli_affected_rows($conn)>0){
		//insert to deposits
		    mysqli_query($conn, "INSERT INTO `sh_sf_deposits` (`user_id`, `transaction_id`, `amount`, `date`, `type`, `status`) VALUES ($usid,'$ref','$net_transfer_amount','$tm','sa_commission','Received')");
			mysqli_query($conn, "INSERT INTO `sh_sf_points_log` (`user_id`, `comment_id`, `amount`, `date`, `type`, `bf`, `af`) VALUES ($usid,'sa_commission credit','$net_transfer_amount','$tm','chips','$chips_balance','$af')");
			if(mysqli_affected_rows($conn)>0){
