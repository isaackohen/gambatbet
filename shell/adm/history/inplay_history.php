<?php error_reporting(0);include('../../db.php');
  ...
  if($_POST['method'] == 'creditrec'){
	$sql = "SELECT * FROM sh_sf_tickets_history ORDER BY date DESC LIMIT 100";
	$prec = "SELECT slip_id FROM sh_sf_tickets_history";
  } else if($_POST['method'] == 'usersmore'){
	$sql = "SELECT * FROM sh_sf_tickets_history ORDER BY date DESC LIMIT $rc, 100";
	$prec = "SELECT slip_id FROM sh_sf_tickets_history";
   }
   
   else if($_POST['method'] == 'singlehist'){
	$sql = "SELECT * FROM sh_sf_tickets_history WHERE user_id = $usid UNION SELECT * FROM sh_sf_slips_history WHERE user_id = $usid ORDER BY date DESC LIMIT 100";
	$result = mysqli_query($conn, "SELECT fname,lname FROM users WHERE id = $usid"); $cr = $result->fetch_assoc();
	$prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE user_id = $usid UNION SELECT slip_id FROM sh_sf_slips_history WHERE user_id = $usid";
	echo '<a id="ksow">Showing Bet History of player '.$cr['fname'].' '.$cr['lname'].'</a>';
  } else if($_POST['method'] == 'singlemore'){
	$sql = "SELECT * FROM sh_sf_tickets_history WHERE user_id = $usid UNION SELECT * FROM sh_sf_slips_history WHERE user_id = $usid ORDER BY date DESC LIMIT $rc, 100";
	$prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE user_id = $usid UNION SELECT slip_id FROM sh_sf_slips_history WHERE user_id = $usid";
   } 
  ...
  $sql = "SELECT * FROM sh_sf_tickets_history WHERE date >= $start AND date <= $end ORDER BY date DESC LIMIT 500";
 $prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE date >= $start AND date <= $end";  
   }
  ...
  $end = strtotime($dt2);
   echo '<div class="timefram">Date Filter '.$dt1.' TO '.$dt2.'</div>';
  $sql = "SELECT * FROM sh_sf_tickets_history WHERE user_id = $usid AND date >= $start AND date <= $end UNION SELECT * FROM sh_sf_slips_history WHERE user_id = $usid AND date >= $start AND date <= $end ORDER BY date DESC LIMIT 500";
  $prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE user_id = $usid AND date >= $start AND date <= $end UNION SELECT slip_id FROM sh_sf_slips_history WHERE user_id = $usid AND date >= $start AND date <= $end";  
   }
   ...
   //search query
    ...
    $idt = $_POST['idt'];
    if (is_numeric($es)){
	 $sql = "SELECT * FROM sh_sf_tickets_history WHERE slip_id = $es OR user_id = $es LIMIT 500";
	 $prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE slip_id = $es OR user_id = $es";
      } else {
  ...
  //delete rows
	...
	foreach($post_ids as $id){ 
     $query = "DELETE FROM sh_sf_tickets_history WHERE slip_id = $id"; 
