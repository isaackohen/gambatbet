<?php error_reporting(0);include('../../db.php');
  ...
  if($_POST['method'] == 'creditrec'){
	$sql = "SELECT * FROM sh_agents_credit_records ORDER BY dt DESC LIMIT 50";
	$prec = "SELECT id FROM sh_agents_credit_records";
  } else if($_POST['method'] == 'usersmore'){
	$sql = "SELECT * FROM sh_agents_credit_records ORDER BY dt DESC LIMIT $rc, 50";
	$prec = "SELECT id FROM sh_agents_credit_records";
   }
   
   else if($_POST['method'] == 'singlehist'){
	$sql = "SELECT * FROM sh_agents_credit_records WHERE agent_id = $usid ORDER BY dt DESC LIMIT 50";
	$result = mysqli_query($conn, "SELECT fname,lname FROM users WHERE id = $usid"); $cr = $result->fetch_assoc();
	$prec = "SELECT id FROM sh_agents_credit_records WHERE agent_id = $usid";
	echo '<a id="ksow">Showing records OF '.$cr['fname'].' '.$cr['lname'].'</a>';
  } else if($_POST['method'] == 'singlemore'){
	$sql = "SELECT * FROM sh_agents_credit_records WHERE agent_id = $usid ORDER BY dt DESC LIMIT $rc, 50";
	$prec = "SELECT id FROM sh_agents_credit_records WHERE agent_id = $usid";
   } 
  ...
  $sql = "SELECT * FROM sh_agents_credit_records WHERE dt >= $start AND dt <= $end ORDER BY dt DESC";
 $prec = "SELECT id FROM sh_agents_credit_records WHERE dt >= $start AND dt <= $end";  
   }
  ...
  $end = strtotime($dt2);
   echo '<div class="timefram">Date Filter '.$dt1.' TO '.$dt2.'</div>';
  $sql = "SELECT * FROM sh_agents_credit_records WHERE agent_id = $usid AND dt >= $start AND dt <= $end ORDER BY dt DESC";
  $prec = "SELECT id FROM sh_agents_credit_records WHERE agent_id = $usid AND dt >= $start AND dt <= $end";  
   }
   ...
   //search query
    ...
    $idt = $_POST['idt'];
    if (is_numeric($es)){
	 $sql = "SELECT * FROM sh_agents_credit_records WHERE sl_id = $es OR agent_id = $es";
	 $prec = "SELECT id FROM sh_agents_credit_records WHERE sl_id = $es OR agent_id = $es";
      } else {
	$sql = "SELECT * FROM sh_agents_credit_records WHERE bet_info LIKE '%".$es."%'";
	$prec = "SELECT id FROM sh_agents_credit_records WHERE bet_info LIKE '%".$es."%'";
	}
  }
  //delete rows
	...
	foreach($post_ids as $id){ 
     $query = "DELETE FROM sh_agents_credit_records WHERE sl_id = $id"; 
...
<input type="hidden" class="ticketvalue" value="50">
 <?php if ($result->num_rows > 0) {
	 while($row = $result->fetch_assoc()) {
      ...
      <td><a href="/admin/agents-credit-history/?usid=<?php echo $row['agent_id'];?>"><i class="icon eye blocked"></i> <?php echo $row['agent_id'];?></a></td>
