<?php error_reporting(0);
 ...
 if(isset($_POST['method']) && $_POST['method'] == 'cricket_cat'){
	...
	function getEventData($event_id,$caton,$term, $conn){
		$catid = $_POST['catid'];
		if($catid == 'Popularc'){
		$query="SELECT * FROM af_inplay_bet_events_cats JOIN af_inplay_bet_options on af_inplay_bet_events_cats.bet_event_cat_id=af_inplay_bet_options.bet_event_cat_id WHERE af_inplay_bet_events_cats.bet_event_id=$event_id AND af_inplay_bet_events_cats.c_sort NOT IN('$caton') ORDER BY c_sort ASC";
		} if($catid == 'Wicketsc'){
			$query="SELECT * FROM af_inplay_bet_events_cats JOIN af_inplay_bet_options on af_inplay_bet_events_cats.bet_event_cat_id=af_inplay_bet_options.bet_event_cat_id WHERE af_inplay_bet_events_cats.bet_event_id=$event_id AND af_inplay_bet_events_cats.bet_event_cat_name LIKE '%".$term."%' ORDER BY c_sort ASC";
		} if($catid == 'Oversc'){
			$query="SELECT * FROM af_inplay_bet_events_cats JOIN af_inplay_bet_options on af_inplay_bet_events_cats.bet_event_cat_id=af_inplay_bet_options.bet_event_cat_id WHERE af_inplay_bet_events_cats.bet_event_id=$event_id AND af_inplay_bet_events_cats.bet_event_cat_name LIKE '%".$term."%' ORDER BY c_sort ASC";
		}else {
		 $query="SELECT * FROM af_inplay_bet_events_cats JOIN af_inplay_bet_options on af_inplay_bet_events_cats.bet_event_cat_id=af_inplay_bet_options.bet_event_cat_id WHERE af_inplay_bet_events_cats.bet_event_id=$event_id AND af_inplay_bet_events_cats.bet_event_cat_name LIKE '%".$term."%' ORDER BY c_sort ASC";	
