...
//function format update slip laybet
 ...
 //function to get odd value of bet option
	...
	return $odo;
        ...
        $qs="SELECT * FROM sh_sf_tickets_records WHERE user_id=$user_id AND status='awaiting' AND bet_info IS NULL AND type='sbook'";
		...
		while($row=$shslips->fetch_assoc()){
			...
			//$okk = $row['bet_option_id']. '' .$row['bet_option_name'];
...
//Fetch on new slip data refresh
  function updateuslip($user_id, $conn){
	...
	if(isset($_POST['get_all_events'])){
		...
		echo json_encode(["event_data"=>$bet_event_data, "odata"=>$odata, "cash_data"=>$cash_data,'unsubmitted_keys'=>$unsubmitted_keys]);
	}
