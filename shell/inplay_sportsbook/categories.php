<?php error_reporting(0);
 ...
 if(isset($_POST['method']) && $_POST['method'] == 'football_cat'){
	...
	//conditions
	if($catid == 'popularf'){
	$catlist = array("Correct Score (Regular Time)", "Total Goals - Exact");
	$caton = join("','",$catlist);
	} else if($catid == 'goals'){
	$catlist = array("Total Goals - Over/Under", "Total Goals O/U - 2nd Half",  "Total Goals O/U - 1st Half");
	$caton = join("','",$catlist);
	} else if($catid == 'handicap'){
	$catlist = array("Handicap 0:1","Handicap 1:0","Handicap 0:2","Handicap 2:0");
	$caton = join("','",$catlist);
	} else if($catid == 'halftime'){
	$catlist = array("Total Goals O/U - 1st Half","1st Goal - 1st Half","Half Time result","Half Time Double Chance");
	$caton = join("','",$catlist);
	} else if($catid == 'scoref'){
	$catlist = array("Correct Score (Regular Time)","Total Goals - Exact");
	$caton = join("','",$catlist);
	} 

	function getEventData($event_id,$caton,$conn){
		$catid = $_POST['catid'];
		if($catid == 'popularf'){
		$query="SELECT * FROM af_inplay_bet_events_cats JOIN af_inplay_bet_options on af_inplay_bet_events_cats.bet_event_cat_id=af_inplay_bet_options.bet_event_cat_id WHERE af_inplay_bet_events_cats.bet_event_id=$event_id AND af_inplay_bet_events_cats.bet_event_cat_name NOT IN('$caton')";
		} else {
		 $query="SELECT * FROM af_inplay_bet_events_cats JOIN af_inplay_bet_options on af_inplay_bet_events_cats.bet_event_cat_id=af_inplay_bet_options.bet_event_cat_id WHERE af_inplay_bet_events_cats.bet_event_id=$event_id AND af_inplay_bet_events_cats.bet_event_cat_name IN('$caton')";	
		}
	...
	$bet_event_data=getEventData($event_id, $caton, $conn);
    ...
    foreach ($bet_event_data['categories'] as $key => $category) {
        ...
        <span class="b_option_name xp">
		  ...
		  <a><?php echo $avaiting_odd_list_back[$okid];?></a> 
		   ...
		   echo '<span class="casout" id="'.$s_back[$okid].'">Cash '.round($ut, 2).'</span>'; endif;?>		  
		  ...
		  <ft class="bm"><?php echo $backrm;?></ft></span>
           </div>   
        </div>		
        <?php }?>
	</div>
