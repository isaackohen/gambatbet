...
include_once('../db.php'); ?>
 ...
 $idt = $_POST['idt'];
 if (is_numeric($es)){
 $sql = "SELECT * FROM sh_sf_tickets_history WHERE slip_id = '$es' OR user_id = '$es'";
  //$prec = mysqli_query($conn, "SELECT DISTINCT bet_option_id FROM sh_sf_slips_history WHERE status = 'awaiting'");
  //$counter = $netcc->num_rows;
 } else {
 $sql = "SELECT * FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed') AND event_name LIKE '%".$es."%'";
 };?>	 
