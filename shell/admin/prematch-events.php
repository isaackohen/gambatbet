...
if(isset($_POST['method']) && $_POST['method'] == 'fsevents'){
   ...
   <div class="esearchview" id="esearchview"></div>
	...
	<?php $xr = "SELECT spid, sname FROM af_pre_bet_events WHERE spid = '$spi'";
	$result = mysqli_query($conn, $xr);
	$c_row = $result->fetch_assoc();
	$tk = $c_row['sname'];
	$tis = $c_row['spid'];
	echo '<h1 class="spnn" id='.$tis.'>'.$tk.'</h1>';
	echo '<input type="hidden" class="cfvalue" value="5"></br></br>';
	$tnow = time();
	$equery = "SELECT * FROM af_pre_bet_events WHERE spid = '$spi' AND $tnow < deadline AND bet_event_id > 100 ORDER BY deadline ASC LIMIT 100";
...
if(isset($_POST['method']) && $_POST['method'] == 'fseventsmore'){
	...
	$tnow = time();
	$equery = "SELECT * FROM af_pre_bet_events WHERE spid = '$spi' AND $tnow < deadline ORDER BY deadline ASC LIMIT $rc, 100";
    $result = $conn->query($equery);
...
if(isset($_POST['method']) && $_POST['method'] == 'edi'){
	$edi = $_POST['edi'];
	$spo = $_POST['spo'];
	echo '<div class="mainedit" id="'.$edi.'" style="display:none">medit</div>';
	...
	$ck = "SELECT is_active, feat FROM af_pre_bet_events WHERE bet_event_id = '$edi'";
	...
	$edix = "SELECT e.bet_event_id, e.sname, e.cc, ec.bet_event_cat_id,ec.bet_event_cat_name,e.deadline, e.spid, e.event_name, e.bet_event_name, o.o_sort, o.bet_option_id, o.bet_option_name, o.bet_option_odd
            ...
            JOIN af_pre_bet_options o ON ec.bet_event_cat_id = o.bet_event_cat_id WHERE e.bet_event_id = $edi ORDER BY ec.c_sort ASC";
...
if(isset($_POST['method']) && $_POST['method'] == 'newedi'){
	...
	$edix = "SELECT * FROM af_pre_bet_events WHERE bet_event_id = $edi";
    $edis = $conn->query($edix);
	//$result = mysqli_query($conn, $xr);
	  $cwc = $edis->fetch_assoc();
      echo '<div class="ediview">';
...
if(isset($_POST['method']) && $_POST['method'] == 'bename'){
	...
	$edibe = "UPDATE af_pre_bet_options SET bet_option_name = '$bename' WHERE bet_option_id = '$beid'";
    $be = $conn->query($edibe);
...
if(isset($_POST['method']) && $_POST['method'] == 'optionna'){
	...
	$edi = "UPDATE af_pre_bet_options SET bet_option_odd = $odv WHERE bet_option_id = $osid";
    $suc = $conn->query($edi);
...
if(isset($_POST['method']) && $_POST['method'] == 'odelete'){
	...
	$did = "DELETE FROM af_pre_bet_options WHERE bet_option_id = $dids";
    $sucd = $conn->query($did);
...
if(isset($_POST['method']) && $_POST['method'] == 'cdelete'){
	...
	$cdik = "DELETE FROM af_pre_bet_events_cats WHERE bet_event_cat_id = $cdil";
	$cdik = $conn->query($cdik);
	$cdil = "DELETE FROM af_pre_bet_options WHERE bet_event_cat_id = $cdil";
...
if(isset($_POST['method']) && $_POST['method'] == 'mevente'){
	$emi = $_POST['emi'];
	$emn = $_POST['emn'];
	$une = "UPDATE af_pre_bet_events SET bet_event_name = '$emn' WHERE bet_event_id = '$emi'";
    $cte = $conn->query($une);
}
...
if(isset($_POST['method']) && $_POST['method'] == 'isact'){
	...
	if($isa == 'Yes'){
	$uni = "UPDATE af_pre_bet_events SET is_active = '0' WHERE bet_event_id = '$iname'";
	$cti = $conn->query($uni);
	$uno = "UPDATE af_pre_bet_events_cats SET yn = '1' WHERE bet_event_id = '$iname'";
	$cto = $conn->query($uno);
	
	} else if($isa == 'No'){
	$uni = "UPDATE af_pre_bet_events SET is_active = '1' WHERE bet_event_id = '$iname'";
	$cti = $conn->query($uni);
    $unn = "UPDATE af_pre_bet_events_cats SET yn = NULL WHERE bet_event_id = '$iname'";
	$ctn = $conn->query($unn);	
	} else {
...
if(isset($_POST['method']) && $_POST['method'] == 'isfeat'){
	...
	if($fsa == 'Yes'){
	$unf = "UPDATE af_pre_bet_events SET feat = 0 WHERE bet_event_id = '$fname'";
	$ctf = $conn->query($unf);
	} else if($fsa == 'No'){
	$unf = "UPDATE af_pre_bet_events SET feat = 1 WHERE bet_event_id = '$fname'";
	$ctf = $conn->query($unf);	
	} else {
		echo 'No actions completed';
	}
...
if(isset($_POST['method']) && $_POST['method'] == 'meventd'){
	...
	$une = "UPDATE af_pre_bet_events SET deadline = '$deadline' WHERE bet_event_id = '$emi'";
    $cte = $conn->query($une);
...
if(isset($_POST['method']) && $_POST['method'] == 'meventl'){
	$emi = $_POST['emi'];
	$emn = $_POST['emn'];
	$une = "UPDATE af_pre_bet_events SET event_name = '$emn' WHERE bet_event_id = '$emi'";
    $cte = $conn->query($une);
}
...
if(isset($_POST['method']) && $_POST['method'] == 'meventc'){
	$emi = $_POST['emi'];
	$emn = $_POST['emn'];
	$une = "UPDATE af_pre_bet_events SET cc = '$emn' WHERE bet_event_id = '$emi'";
    $cte = $conn->query($une);
}
...
if(isset($_POST['method']) && $_POST['method'] == 'catedit'){
	...
	$edicat = "UPDATE af_pre_bet_events_cats SET bet_event_cat_name = '$ctname' WHERE bet_event_cat_id = '$ctid'";
...
if(isset($_POST['method']) && $_POST['method'] == 'addingev'){
	...
	$ecc = $_POST['ecc'];
	$xxr = "SELECT DISTINCT event_id FROM af_pre_bet_events WHERE event_name = '$ligname'";
	...
	$eventid = $xc_row['event_id'];
	 ...
	 $evin = "INSERT INTO af_pre_bet_events (bet_event_id, b_sort, bet_event_name, deadline, is_active, feat, event_id, event_name, spid, cc, sname) VALUES('$beventid', '0', '$beventname', '$deadline', '2', '0', '$eventid', '$ligname', '$spid', '$ecc', '$spname')";
...
if(isset($_POST['method']) && $_POST['method'] == 'addcat'){
	...
	$c_count = "SELECT count(*) as count FROM af_pre_bet_events_cats WHERE bet_event_id = '$evid'";
	 ...
	 $catin = "INSERT INTO af_pre_bet_events_cats (bet_event_cat_id, c_sort, bet_event_cat_name, bet_event_id) VALUES('$catid', '$c_sort', '$catn', '$evid')";
...
if(isset($_POST['method']) && $_POST['method'] == 'soptions'){
	...
	$c_count = "SELECT count(*) as count FROM af_pre_bet_options WHERE bet_event_cat_id = '$catid'";
			 $c_result = $conn->query($c_count);
                            if ($c_result->num_rows > 0) {
                                $c_row = $c_result->fetch_assoc();
	 ...
	 $opin = "INSERT INTO af_pre_bet_options (bet_option_id, o_sort, bet_option_name, bet_option_odd, status,  bet_event_cat_id) VALUES('$opid', '$o_sort', '$opname', '$optionod', 'awaiting', '$catid')";
...
if(isset($_POST['method']) && $_POST['method'] == 'addopp'){
	...
	$okid = $_POST['okid'];
	$opid = rand(1000000, 9999999);
	$c_count = "SELECT count(*) as count FROM af_pre_bet_options WHERE bet_event_cat_id = '$okid'";
			 $c_result = $conn->query($c_count);
                            if ($c_result->num_rows > 0) {
                                $c_row = $c_result->fetch_assoc();
                                $o_sort = $c_row['count']+1;
	 ...
	 $opin = "INSERT INTO af_pre_bet_options (bet_option_id, o_sort, bet_option_name, bet_option_odd, status,  bet_event_cat_id) VALUES('$opid', '$o_sort', '$opname', '$optionod', 'awaiting', '$okid')";
	 if ($conn->query($opin) === TRUE) {
		//echo trim($catid);
			 } else {
				 //echo "Could not save";
...
$sql_pre = "SELECT * FROM af_pre_bet_events LIMIT ".$diff."";
