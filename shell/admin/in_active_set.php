...
include_once('../db.php');
 $rc = $_POST['rc'];
 $prec = "SELECT DISTINCT bet_option_id FROM sh_sf_tickets_records WHERE status = 'awaiting'";
  ...
  if($_POST['method'] == 'prematch_active'){
    $sql = "SELECT MIN(bet_option_id), slip_id, user_id,status, stake, winnings,date, bet_info, odd, event_id, event_name,cat_id,cat_name,bet_option_id, bet_option_name, aid, type,sp,debit FROM sh_sf_tickets_records WHERE status = 'awaiting' GROUP BY bet_option_id ORDER BY date DESC LIMIT 200";
  } else if($_POST['method'] == 'prelodo'){
    $sql = "SELECT MIN(bet_option_id), slip_id, user_id,status, stake, winnings,date, bet_info,odd, event_id, event_name,cat_id,cat_name,bet_option_id, bet_option_name, aid, type,sp,debit FROM sh_sf_tickets_records WHERE status = 'awaiting' GROUP BY bet_option_id ORDER BY date DESC LIMIT $rc, 200";
	};
