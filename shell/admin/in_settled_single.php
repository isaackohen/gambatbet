...
include_once('../db.php'); ?>
 ...
 $prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed')";
 }else{
 $prec = "SELECT slip_id FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed') AND type = '$ttype' OR status NOT IN('awaiting', 'trashed') AND type = '$lomo'";	 
 }
  $netcc = mysqli_query($conn, $prec);
  $counter = $netcc->num_rows;
  $cctr = "SELECT slip_id FROM sh_sf_tickets_history WHERE status = 'trashed'";
 ...
 if($ttype == 'allevents' || $lomo == 'allevents'){  
  if($_POST['method'] == 'typesingle'){
    $sql = "SELECT * FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed') ORDER BY date DESC LIMIT 100";
  } else if($_POST['method'] == 'prelodo'){
    $sql = "SELECT * FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed') ORDER BY date DESC LIMIT $rc, 100";
	}
 }else {
   if($_POST['method'] == 'typesingle'){
    $sql = "SELECT * FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed') AND type = '$ttype' ORDER BY date DESC LIMIT 100";
  } else if($_POST['method'] == 'prelodo'){
    $sql = "SELECT * FROM sh_sf_tickets_history WHERE status NOT IN('awaiting', 'trashed') AND type = '$lomo' ORDER BY date DESC LIMIT $rc, 100";
	}	 
 }
