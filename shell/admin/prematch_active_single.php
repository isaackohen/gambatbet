...
include_once('../db.php'); ?>
 ...
 $prec = "SELECT slip_id FROM sh_sf_slips_history WHERE status = 'awaiting'";
 }else{
 $prec = "SELECT slip_id FROM sh_sf_slips_history WHERE status = 'awaiting' AND type = '$ttype' OR status = 'awaiting' AND type = '$lomo'";	 
 }
 ...
 if($ttype == 'allevents' || $lomo == 'allevents'){  
  if($_POST['method'] == 'typesingle'){
    $sql = "SELECT * FROM sh_sf_slips_history WHERE status = 'awaiting' ORDER BY date DESC LIMIT 100";
  } else if($_POST['method'] == 'prelodo'){
    $sql = "SELECT * FROM sh_sf_slips_history WHERE status = 'awaiting' ORDER BY date DESC LIMIT $rc, 100";
	}
 }else {
   if($_POST['method'] == 'typesingle'){
    $sql = "SELECT * FROM sh_sf_slips_history WHERE status = 'awaiting' AND type = '$ttype' ORDER BY date DESC LIMIT 100";
  } else if($_POST['method'] == 'prelodo'){
    $sql = "SELECT * FROM sh_sf_slips_history WHERE status = 'awaiting' AND type = '$lomo' ORDER BY date DESC LIMIT $rc, 100";
	}	 
 }
