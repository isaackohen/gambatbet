...
include_once('../db.php'); ?>
 ...
 <?php 
 $rc = $_POST['rc'];
 $prec = "SELECT DISTINCT bet_option_id FROM sh_sf_slips_history WHERE status NOT IN('awaiting', 'trashed')";
  ...
  if($_POST['method'] == 'prematch_active'){
    $sql = "SELECT * FROM sh_sf_slips_history WHERE status NOT IN('awaiting', 'trashed') GROUP BY bet_option_id ORDER BY date DESC LIMIT 100";
  } else if($_POST['method'] == 'prelodo'){
    $sql = "SELECT * FROM sh_sf_slips_history WHERE status NOT IN('awaiting', 'trashed') GROUP BY bet_option_id ORDER BY date DESC LIMIT $rc, 100";
	};
