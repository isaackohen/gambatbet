	...
	if($winnings > $stake){
		...
		if($type == 'chips'){
			...
			//if(mysqli_affected_rows($conn) > 0){
				mysqli_query($conn, "INSERT INTO `sh_virtual_games` (`user_id`, `agent_id`, `game_name`, `stake`, `user_win`, `ag_win`, `type`, `updated_at`) VALUES ($usid, '$afid', 'Virtual Greyhound', '$stake', '$tkwin', '$netagwin', '$type', $updated_at)");
		...
		//for promo balance.. no agent credit for promo
			...
			//if(mysqli_affected_rows($conn) > 0){
				mysqli_query($conn, "INSERT INTO `sh_virtual_games` (`user_id`, `agent_id`, `game_name`, `stake`, `user_win`, `ag_win`, `type`, `updated_at`) VALUES ($usid, '$afid', 'Virtual Greyhound', '$stake', '$tkwin', '$agwin', '$type', $updated_at)");
