...
//mysqli_query($conn,"DELETE FROM af_pre_bet_events WHERE is_active = '1' AND feat = '0'");
    ...
    $url='https://api.betsapi.com/v1/bwin/event?token='.$tk.'&event_id=201040776';
    $ch = curl_init($url);
        ...
        for($i =0; $i<count($b); $i++){
            ...
            if(!empty($b[$i]['Id']) && !empty($bradar)){
			 ...
			 //INSERT BET EVENTS CATS
              ...
              for($j =0; $j<count($c); $j++){
				  $c_sort = 1;
				  echo $bet_event_cat_id = $c[$j]['id'];echo '</br>';
			...
			if($visible == 'Visible'){				
			 ...
			 $d = $c[$j]['options'];
               foreach ($d as $e) {
				   $bet_option_id = $e['price']['id'];
                   $oname = $e['name']['value'];
                   $bet_option_name = addslashes($oname );
                   $bet_option_odd = $e['price']['odds'];
				   $ovisible = $e['status'];
