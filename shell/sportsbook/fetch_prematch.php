<?php error_reporting(0);
 ...
 if(isset($_POST['method']) && $_POST['method'] == 'football_cat'){
	...
	$catlist = array("Total Goals - Over/Under");
	$caton = join("','",$catlist);
	...
	function getEventData($event_id,$caton,$conn){
		$catid = $_POST['catid'];
		if($catid == 'popularf'){
		$query="SELECT * FROM af_pre_bet_events_cats JOIN af_pre_bet_options on af_pre_bet_events_cats.bet_event_cat_id=af_pre_bet_options.bet_event_cat_id WHERE af_pre_bet_events_cats.bet_event_id=$event_id AND af_pre_bet_events_cats.bet_event_cat_name NOT IN('$caton')";
		} else {
		 $query="SELECT * FROM af_pre_bet_events_cats JOIN af_pre_bet_options on af_pre_bet_events_cats.bet_event_cat_id=af_pre_bet_options.bet_event_cat_id WHERE af_pre_bet_events_cats.bet_event_id=$event_id AND af_pre_bet_events_cats.bet_event_cat_name IN('$caton')";	
		}
...
//function format update slip laybet
  ...
  return $lodk;
     ...
     if(empty($bet_event_data['categories'])){
		 echo '<div class="opun">Options unavailable for bet</div>';
		 die();
	 }
