...
$shsl = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM sh_sf_slips WHERE slip_id = $slipid AND status = 'awaiting' AND bet_info IS NULL AND type = 'sbook'"));
...
$sql = "SELECT * from sh_sf_slips_history WHERE user_id=$user_id AND bet_info IS NULL AND bet_option_id = $_option_id AND bet_option_name = '$_option_name' AND status = 'awaiting' AND type = 'sbook'";
		...
		if(mysqli_affected_rows($conn) > 0){
			...
			if(!empty($said)){
				...
				if(mysqli_affected_rows($conn) > 0){
				 ...
				 mysqli_query($conn,"DELETE FROM sh_sf_slips WHERE user_id=$user_id AND bet_option_id=$_option_id AND bet_option_name='$_option_name' AND type='sbook'"); 
				}else{
					echo 'Suspended';
					//echo $conn->error;
				}
